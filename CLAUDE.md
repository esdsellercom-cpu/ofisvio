...
- **Masa/ofis envanteri** `spaces` (saatlik odalar `rooms`), tahsis yalnız `SpaceService::assign/end` (alan satır kilidi + kapasite; audit); doluluk `SpaceService::occupancy`. Oda/alan/lokasyon bakım durumu `maintenance_until/_note` (`HasMaintenanceStatus`); olanaklar `amenities`; kapak `cover_media_id` yalnız lokasyon galerisinden.
