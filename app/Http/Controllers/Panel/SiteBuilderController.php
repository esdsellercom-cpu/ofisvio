...
class SiteBuilderController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        return view('panel.site-builder.index', [
            ...
            // Blog bölümü paneli (faz 58): yayındaki yazılar (öne çıkanlar önce) — kart seçilince yazıya gidiş.
            'posts' => $website ? $this->contents->homePosts($website)->map(fn ($p) => ['id' => $p->id, 'title' => $p->title, 'featured' => (bool) $p->is_featured, 'cover' => (string) $p->cover_url !== ''])->all() : [],
