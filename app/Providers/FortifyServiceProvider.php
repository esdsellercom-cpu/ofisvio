...
class FortifyServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        // Askıya alınmış hesap (users.status) doğru şifreyle de giremez; ayrım sızdırmamak için genel hata.
        Fortify::authenticateUsing(function (Request $request) {
            $user = User::query()->where('email', $request->input(Fortify::username()))->first();

            if ($user !== null && ! $user->isSuspended() && Hash::check((string) $request->input('password'), $user->password)) {
                return $user;
            }
