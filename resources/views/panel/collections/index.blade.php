...
@section('content')
    <div class="panel-head">
        ...
        <div class="panel-head__actions">
            ...
            @can('payment_allocation.manage')
                ...
                <a href="#tahsilatlar" class="btn btn--ghost">Tahsilat makbuzu</a>
                <a href="#acik-faturalar" class="btn btn--ghost">Geciken ödeme belgesi</a>
            @endcan
            <a href="#belgeler" class="btn btn--ghost">Belge şablonları</a>
    ...
    <div class="stack" style="gap:18px">
        ...
        <div class="grid g-2-1">
            <div class="card" id="acik-faturalar">
                    ...
                    <div class="tw"><table class="t"><thead><tr><th>Fatura</th><th>Şirket</th><th>Vade</th><th class="num">Kalan</th><th></th></tr></thead><tbody>
                        @foreach ($open as $inv)
                            <tr>
                                ...
                                <td class="num"><span style="display:inline-flex;gap:4px;justify-content:flex-end;flex-wrap:wrap">
                                    @can('payment_allocation.manage')
                                        {{-- Faz 47: satır aksiyonları — mevcut "Aç" korunur --}}
                                        <button type="button" class="btn btn--quiet" data-modal-open="#modal-payment" data-title="Tahsilat · {{ $inv->number }}" data-fill="{{ json_encode(['company_id' => $inv->company_id, 'invoice_id' => $inv->id, 'amount' => \App\Support\Money::major($inv->outstanding()), 'description' => $inv->description, 'paid_on' => now()->toDateString()]) }}">Tahsilat ekle</button>
                                        @if ($inv->status === 'overdue' || $inv->daysOverdue() > 0)
                                            <form method="POST" action="{{ route('panel.collections.invoices.notice', $inv->id) }}">@csrf<button type="submit" class="btn btn--quiet">Belge oluştur</button></form>
                                        @endif
                                    @endcan
        ...
        {{-- Faz 47: mevcut yapının altına eklenen bölümler — tahsilat kayıtları (makbuz/iptal) ve belgeler/şablonlar --}}
        <div id="tahsilatlar">@include('panel.collections.partials.section-payments')</div>
        <div id="belgeler">@include('panel.collections.partials.section-documents')</div>
    </div>
