    ...
    <form method="POST" action="{{ $content ? route('panel.content.update', $content) : route('panel.content.store') }}"
        ...
        <div class="panel stack" style="gap:14px">
            <label class="field">
                ...
                <input class="control" type="text" name="title" value="{{ old('title', $content?->title) }}" required minlength="3" maxlength="190" autofocus
                       @error('title') aria-invalid="true" @enderror>
            </label>

            <label class="field">
                <span class="label">Slug (boş bırakılırsa başlıktan üretilir)</span>
                <input class="control mono" type="text" name="slug" value="{{ old('slug', $content?->slug) }}" maxlength="190" pattern="[a-z0-9-]*"
                       placeholder="{{ $kind->value === 'post' ? 'blog/ornek-yazi' : 'ornek-sayfa' }}" @error('slug') aria-invalid="true" @enderror>
            </label>

            <label class="field">
                <span class="label">Özet</span>
                <textarea class="control" name="excerpt" maxlength="500" style="min-height:72px" @error('excerpt') aria-invalid="true" @enderror>{{ old('excerpt', $content?->excerpt) }}</textarea>
            </label>

            <label class="field">
                <span class="label">Gövde (Markdown)</span>
                <textarea class="control mono" name="body" style="min-height:420px;font-size:14px;line-height:1.6" @error('body') aria-invalid="true" @enderror>{{ old('body', $content?->body) }}</textarea>
            ...
            <div class="panel stack" style="gap:14px">
                ...
                @if ($kind->value === 'post')
                    <label class="field">
                        ...
                        <input class="control" type="text" name="category" value="{{ old('category', $content?->category) }}" maxlength="80" placeholder="Sanal Ofis, Mevzuat, Hibrit Çalışma…">
                    </label>
                @endif
