        ...
        <div class="card__body">
            ...
            @if ($companyId > 0)
                <form method="POST" action="{{ route('panel.invoices.store') }}" class="stack" style="gap:12px">
                    ...
                    <div class="grid g3">
                        <label class="field"><span class="label">Ara toplam ({{ money_symbol() }})</span><input class="control" type="number" step="0.01" name="subtotal" value="{{ old('subtotal', '0.00') }}" min="0" required @error('subtotal') aria-invalid="true" @enderror>@error('subtotal')<span class="field-error">{{ $message }}</span>@enderror</label>
