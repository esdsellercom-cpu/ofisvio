...
<section @if ($anchor) id="{{ $anchor }}" @endif class="dark-band" style="margin-top:96px">
    <div class="wrap" style="padding-block:78px;display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:52px;align-items:start">
        <div style="min-width:0">
            ...
            <h2 class="h2" style="max-width:22ch"{!! ofv($s, 'title', 'texts.meeting_title') !!}>{{ $s['title'] ?? $texts['meeting_title'] }}</h2>
            <p class="lede" style="margin:24px 0 0;max-width:46ch;font-size:17.5px"{!! ofv($s, 'lede', 'texts.meeting_lede') !!}>{{ $s['lede'] ?? $texts['meeting_lede'] }}</p>
