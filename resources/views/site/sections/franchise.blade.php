...
@php($points = collect((array) ($s['points'] ?? $d['points']))->map(fn ($p) => trim((string) $p))->filter()->values())
            ...
            @if ($points->isNotEmpty())
                <ul class="stack" style="gap:8px;margin:6px 0 0;padding:0;list-style:none">
                    @foreach ($points as $point)
                        <li style="display:flex;gap:10px;align-items:flex-start;font-size:15px"><span style="width:8px;height:8px;border-radius:99px;background:var(--brand);flex:none;margin-top:7px"></span><span>{{ $point }}</span></li>
                    @endforeach
                </ul>
            @endif
            @if ($cta)
