...
<section @if ($anchor) id="{{ $anchor }}" @endif class="wrap section--tight sec-inner">
    <figure class="content-img" style="margin:0 auto;width:{{ $width }}%">
        @if ($img)
            ...
            <img src="{{ $img['url'] }}" alt="{{ $img['alt'] }}" loading="lazy" decoding="async" style="width:100%;{{ $ratio }}object-fit:{{ ($s['fit'] ?? 'cover') === 'contain' ? 'contain' : 'cover' }};border-radius:var(--sec-radius, var(--r-lg));display:block" @if (ofv_editor()) data-ofv-image="media" @endif>
            @if (! empty($s['link']))</a>@endif
        @else
            <div class="shot" style="{{ $ratio ?: 'aspect-ratio:16/9;' }}border-radius:var(--r-lg);border:1px dashed var(--line);display:flex;align-items:center;justify-content:center;color:var(--ink-faint)" @if (ofv_editor()) data-ofv-image="media" @endif>Görsel seçilmedi</div>
        @endif
