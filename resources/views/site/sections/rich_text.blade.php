...
<section @if ($anchor) id="{{ $anchor }}" @endif class="wrap section">
    ...
    @if (! empty($s['title']))<h2 class="h2" style="max-width:26ch">{{ $s['title'] }}</h2>@endif
    @if (! empty($s['body']))<div class="prose" style="margin-top:22px;max-width:70ch">{!! \Illuminate\Support\Str::markdown((string) $s['body'], ['html_input' => 'strip', 'allow_unsafe_links' => false]) !!}</div>@endif
